...
$('#mytab').bootstrapTable({
    ...
    url: "/item/houseOthersItemList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            formatter: function (value, row, index) {
                if (value == 0) {
                    //表示启用状态
                    return '<i  style="color:green">已结算</i>';
                } else {
                    //表示启用状态
        ...
        //     field: '',
        //     formatter: function (value, row, index) {
        //         var e = '<a title="编辑" href="javascript:void(0);" id="item"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function edit(name) {
    $.post("/item/findItem/" + name,
        function (data) {
            //刷新列表
            $("#updateform").autofill(data);
            var colum = $("#masterById").select2();
            //选中某一行
            colum.val(data.itemMasterVo.id).trigger("change");
            colum.change();
            $("#itemDate").val(formatTime(data.itemDate));
...
function updatestatus(id, status) {
    $.post("/item/updateStatus/" + id + "/" + status,
        function (data) {
            if (status == 0) {
                if (data.message == "ok") {
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/item/itemList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/item/itemList'});
}
