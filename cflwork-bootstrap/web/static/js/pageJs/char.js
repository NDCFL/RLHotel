    ...
    columns: [
        ...
        },
        {
            ...
            formatter: function (value, row, index) {
                if(value.length<10){
                    return '<a   data-toggle="modal" title="点击查看详情" alt="点击查看详情" data-id="\'' + row.id + '\'" data-target="#remark_modal" onclick="return remark(\'' + value + '\')">'+value.substr(0,10)+'</a>';
                }else{
                    return '<a   data-toggle="modal" title="点击查看详情" alt="点击查看详情" data-id="\'' + row.id + '\'" data-target="#remark_modal" onclick="return remark(\'' + value + '\')">'+value.substr(0,10)+"..."+'</a>';
                }
            }
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="char"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#myModal" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
...
function updatestatus(id, status) {
    $.post("/char/updateStatus/" + id + "/" + status,
        function (data) {
            if(status==0){
                if(data.message=="ok"){
...
$("#updateSta").click(function () {
    layer.confirm('确认要执行批量修改商会论坛状态吗？',function(index){
        $.post(
            "/char/deleteManyChar",
            {
                "manyId":$("#statusId").val(),
