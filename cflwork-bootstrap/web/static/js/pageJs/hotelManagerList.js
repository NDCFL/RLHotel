...
$('#mytab').bootstrapTable({
    ...
    url: "/hotelManager/hotelManagerList",//要请求数据的文件路径
    ...
    columns: [
        ...
        ,
        {
            ...
            field: '',
            formatter: function (value, row, index) {
                var e = '<a title="编辑" href="javascript:void(0);" id="cashSubject"  data-toggle="modal" data-id="\'' + row.id + '\'" data-target="#updateLan" onclick="return edit(\'' + row.id + '\')"><i class="glyphicon glyphicon-pencil" alt="修改" style="color:green">修改</i></a> ';
                ...
                var g = '<a title="初始化密码" href="javascript:void(0);" onclick="initPwd(' + row.id+')"><i class="glyphicon glyphicon-phone" style="color:green"></i>重置密码</a>'
...
function del(hotelManager, status) {
    if (status == 0) {
    ...
    layer.confirm('确认要删除吗？', function (index) {
        $.ajax({
            type: 'POST',
            url: '/hotelManager/deleteHotelManager/' + hotelManager,
            dataType: 'json',
            success: function (data) {
...
function edit(name){
    $.post("/hotelManager/findHotelManager/"+name,
        function(data){
...
function updatestatus(id, status) {
    $.post("/hotelManager/updateStatus/" + id + "/" + status,
        function (data) {
            if (status == 0) {
                if (data.message == "ok") {
...
$('#search_btn').click(function () {
    $('#mytab').bootstrapTable('refresh', {url: '/hotelManager/hotelManagerList'});
})
function refush() {
    $('#mytab').bootstrapTable('refresh', {url: '/hotelManager/hotelManagerList'});
}
$("#update").click(function () {
    $.post(
        "/hotelManager/hotelManagerUpdateSave",
        $("#updateform").serialize(),
        function (data) {
            layer.alert(data.message, {icon: 6});
            refush();
            $("#updateLan").modal('hide');
        }, "json"
    );
});
function deleteMany() {
